...
class Admin(commands.Cog):
    ...
    async def channel(self, inter: disnake.ApplicationCommandInteraction, channel: disnake.TextChannel) -> None:
        try:
            self.bot.redis.set(f"{inter.guild.id}:channel", channel.id)
            embed = Embed.success(
                inter.guild,
    ...
    @commands.slash_command(description="Ruins the count if any message is sent if it is not either a valid number or bot command.")
    @commands.has_permissions(administrator=True, manage_guild=True)
    async def numbersonly(self, inter: disnake.ApplicationCommandInteraction, enabled: bool) -> None:
        try:
            self.bot.redis.set(f"{inter.guild.id}:numbersonly", str(enabled))
            embed = Embed.success(
                inter.guild,
                f"Numbers only {'enabled' if enabled else 'disabled'}!"
            )
            await inter.response.send_message(embed=embed)
